...
# ============================================================================
# Auto-Run Logic: If no parameters provided, run both rtp off and tp off
# ============================================================================
if (-not $PSBoundParameters.ContainsKey('Command') -and -not $PSBoundParameters.ContainsKey('Action')) {
    Write-Host "=== Auto-Run Mode: Executing both 'rtp off' and 'tp off' ===" -ForegroundColor Cyan
    Write-Host ""
    
    # Run rtp off
    Write-Host "[1/2] Setting Real-Time Protection to OFF..." -ForegroundColor Yellow
    & $PSCommandPath -Command 'rtp' -Action 'off'
    $rtpResult = $LASTEXITCODE
    
    Write-Host ""
    Write-Host "[2/2] Setting Tamper Protection to OFF..." -ForegroundColor Yellow
    & $PSCommandPath -Command 'tp' -Action 'off'
    $tpResult = $LASTEXITCODE
    
    Write-Host ""
    Write-Host "=== Auto-Run Complete ===" -ForegroundColor Cyan
    Write-Host "Real-Time Protection: $(if ($rtpResult -eq 0) { 'SUCCESS' } else { 'FAILED' })"
    Write-Host "Tamper Protection: $(if ($tpResult -eq 0) { 'SUCCESS' } else { 'FAILED' })"
    
    # Exit with worst result code
    exit $(if ($rtpResult -eq 0 -and $tpResult -eq 0) { 0 } else { 1 })
}
...
# ============================================================================
# Main Execution Flow (only executed when parameters are provided)
# ============================================================================
